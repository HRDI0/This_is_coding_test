...
# 바다로 되어 있는 공간에는 갈 수 없다. 캐릭터의 움직임을 설정하기 위해 정해 놓은 매뉴얼은 이러하다.
...
#2차원 배열에서의 동서남북의 이동의 수치적 변화를 기억하자.
#북쪽으로 이동한다는 것은 이론적으로 y값을 -1 하는 것이지만 배열의 좌표값상에서는 1,1 일 경우 0,1 로 가는 것이다. 
# 즉 [x][y]로 생각하면 안된다. [y(상식적 좌표변화)][x(상식적 좌표변화)] 인 것이다. 아래 코드를 보면 이해 할수 있다.
#   북
#서    동           <- 방위 //헷갈리지 마 좀...
#   남
...
sample_map = []
for _ in range(n):
    sample_map.append(list(map(int,input().split())))
...
while(True):
    ...
    if side == 0:
        if sample_map[y-1][x] == 0:
            y -=1
            sample_map[y][x] = 2
            result +=1
    elif side == 1:
        if sample_map[y][x+1] == 0:
            x +=1
            sample_map[y][x] = 2
            result +=1
    elif side == 2:
        ...
        if sample_map[y][x-1] == 0:
            ...
            sample_map[y][x] = 2
            result +=1
    if sample_map[y][x+1] !=0 and sample_map[y][x-1] !=0 and sample_map[y+1][x] != 0 and sample_map[y-1][x] !=0:
        if side == 0:
            if sample_map[y+1][x] != 1:
                y +=1
            else: break
        elif side == 1:
            if sample_map[y][x-1] != 1:
                x -=1
            else: break
        elif side == 2:
            if sample_map[y-1][x] != 1:
